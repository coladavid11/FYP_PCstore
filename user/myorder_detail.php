...
.tl-dot.done    { background:#d4af37; border-color:#d4af37; color:#000; }
.tl-dot.current { background:#0f0f0f; border-color:#d4af37; color:#d4af37; box-shadow:0 0 0 4px rgba(212,175,55,0.15); }
.tl-dot.cancel  { background:rgba(220,53,69,0.1); border-color:#dc3545; color:#dc3545; }
...
.tl-label.done    { color:#d4af37; font-weight:600; }
.tl-label.current { color:#d4af37; font-weight:700; }
...
.item-price .unit  { font-size:0.75rem; color:#555; }
.item-price .total { font-size:0.95rem; font-weight:700; color:#d4af37; }
...
.info-cell .ic-label { font-size:0.7rem; color:#555; text-transform:uppercase; letter-spacing:1px; margin-bottom:4px; }
.info-cell .ic-val   { font-size:0.88rem; color:#ccc; font-weight:500; }
...
.pay-icon { width:44px; height:44px; border-radius:8px; display:flex; align-items:center; justify-content:center; font-size:1.3rem; }
...
.btn-act-gold    { background:linear-gradient(45deg,#d4af37,#c5a028); color:#000; }
.btn-act-gold:hover { background:#fff; color:#000; transform:translateY(-2px); }
.btn-act-outline { background:transparent; border:1px solid #2a2a2a; color:#aaa; }
.btn-act-outline:hover { border-color:#555; color:#fff; }
.btn-act-danger  { background:rgba(220,53,69,0.1); border:1px solid rgba(220,53,69,0.3); color:#dc3545; }
.btn-act-danger:hover  { background:rgba(220,53,69,0.2); }
.btn-act-info    { background:rgba(23,162,184,0.1); border:1px solid rgba(23,162,184,0.3); color:#17a2b8; }
.btn-act-info:hover    { background:rgba(23,162,184,0.2); }

/* ── BREADCRUMB ── */
.breadcrumb-dark { display:flex; gap:8px; align-items:center; margin-bottom:28px; }
.breadcrumb-dark a { color:#555; text-decoration:none; font-size:0.8rem; }
.breadcrumb-dark a:hover { color:#d4af37; }
.breadcrumb-dark span { color:#333; font-size:0.8rem; }
.breadcrumb-dark .sep { color:#222; }

/* ── INVOICE PRINT STYLES ── */
@media print {
    header, footer, .btn-act, .breadcrumb-dark, #actionBar, #trackingPanel { display:none !important; }
    body { background:#fff; color:#000; }
    .panel { border:1px solid #ddd; border-radius:0; }
    .panel-title, .ic-label, .item-meta { color:#666 !important; }
    .item-name, .ic-val, .summary-row { color:#000 !important; }
    .summary-row.grand { color:#b8860b !important; }
}
</style>
</head>
<body>
...
<div class="container py-5">
    ...
    <div class="d-flex justify-content-between align-items-start flex-wrap gap-3 mb-4">
        <div>
            <div style="font-size:0.72rem;color:#555;text-transform:uppercase;letter-spacing:1.5px;margin-bottom:4px;">Order Details</div>
            <h2 style="font-family:'Playfair Display',serif; margin:0;"><?php echo htmlspecialchars($order['order_number']); ?></h2>
    ...
    <div id="actionBar" class="d-flex flex-wrap gap-2 mb-4">
        ...
        <button class="btn-act btn-act-danger" onclick="cancelOrder(<?php echo $order_id; ?>, '<?php echo htmlspecialchars($order['order_number']); ?>')">
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-8">
            ...
            <div class="panel" style="border-color:rgba(220,53,69,0.25);">
                <div style="display:flex; align-items:center; gap:14px;">
                    <div style="width:48px;height:48px;border-radius:50%;background:rgba(220,53,69,0.1);border:1px solid rgba(220,53,69,0.3);display:flex;align-items:center;justify-content:center;color:#dc3545;font-size:1.2rem;flex-shrink:0;">
                        <i class="fa fa-times"></i>
                    </div>
                    <div>
                        <div style="font-weight:600;color:#dc3545;margin-bottom:2px;">Order Cancelled</div>
                        <div style="font-size:0.82rem;color:#555;">This order has been cancelled. If you were charged, a refund will be processed within 3–5 business days.</div>
                    </div>
                </div>
            </div>
            <?php endif; ?>
            ...
            <!-- ═══ ORDER ITEMS ═══ -->
            <div class="panel">
                <div class="panel-title"><i class="fa fa-box-open"></i> Order Items (<?php echo count($items); ?>)</div>
        ...
        <div class="col-lg-4">
            ...
            <div class="panel">
                ...
                <div class="pay-card">
                    <div class="pay-icon" style="background:rgba(<?php echo $isCredit ? '212,175,55' : '23,162,184'; ?>,0.1);color:<?php echo $payColor; ?>;">
                        <i class="fa <?php echo $payIcon; ?>"></i>
                    </div>
                    <div>
                        <div style="font-size:0.75rem;color:#555;text-transform:uppercase;letter-spacing:1px;margin-bottom:2px;">Method</div>
                        <div style="font-weight:600;color:#ccc;"><?php echo htmlspecialchars($order['payment_method']); ?></div>
                    </div>
                    <div class="ms-auto">
                        <span style="font-size:0.75rem; font-weight:700; color:<?php echo $paymentCfg['color']; ?>;">
            ...
            <div class="panel">
                ...
                <div class="info-grid">
                    ...
                    <div class="info-cell">
                        ...
                        <div class="ic-val" style="font-size:0.8rem;"><?php echo htmlspecialchars($order['order_number']); ?></div>
                    </div>
                    <div class="info-cell">
...
<div class="modal-content" style="background:#121212;border:1px solid #2a2a2a;border-radius:12px;">
    ...
    <div class="modal-header" style="border-color:#1e1e1e;">
        ...
        <div class="d-flex gap-2">
            <button onclick="printInvoice()" class="btn-act btn-act-gold" style="padding:6px 14px;font-size:0.75rem;">
    ...
    <div class="modal-body" id="invoicePrintArea" style="background:#fff;color:#000;padding:32px 40px;">
        ...
        <!-- INVOICE HEADER -->
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:30px;padding-bottom:20px;border-bottom:2px solid #000;">
            <div>
                <div style="font-size:1.6rem;font-weight:800;letter-spacing:1px;color:#000;">MY PC STORE</div>
                <div style="font-size:0.75rem;color:#666;text-transform:uppercase;letter-spacing:1px;">Premium Gaming Hardware</div>
            </div>
            <div style="text-align:right;">
                <div style="font-size:1.1rem;font-weight:700;color:#b8860b;">INVOICE</div>
                <div style="font-size:0.85rem;color:#333;margin-top:4px;"><?php echo htmlspecialchars($order['order_number']); ?></div>
                <div style="font-size:0.8rem;color:#666;">Date: <?php echo date('d M Y', strtotime($order['created_at'])); ?></div>
            </div>
        </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:24px;">
            <div>
                <div style="font-size:0.65rem;text-transform:uppercase;letter-spacing:1px;color:#999;margin-bottom:4px;">Bill To</div>
                <div style="font-weight:600;color:#333;"><?php echo htmlspecialchars($_SESSION['name'] ?? 'Customer'); ?></div>
                <div style="font-size:0.8rem;color:#666;"><?php echo htmlspecialchars($_SESSION['email'] ?? ''); ?></div>
            </div>
            <div>
                <div style="font-size:0.65rem;text-transform:uppercase;letter-spacing:1px;color:#999;margin-bottom:4px;">Payment</div>
                <div style="font-weight:600;color:#333;"><?php echo htmlspecialchars($order['payment_method']); ?></div>
                <div style="font-size:0.8rem;color:#666;">Status: <?php echo htmlspecialchars($order['payment_status'] ?? 'N/A'); ?></div>
            </div>
        </div>
        ...
        <table style="width:100%;border-collapse:collapse;margin-bottom:20px;">
            <thead>
                <tr style="background:#f5f5f5;border-bottom:2px solid #ddd;">
                    <th style="padding:10px 12px;text-align:left;font-size:0.75rem;text-transform:uppercase;letter-spacing:1px;color:#666;">Product</th>
                    <th style="padding:10px 12px;text-align:center;font-size:0.75rem;text-transform:uppercase;letter-spacing:1px;color:#666;">Qty</th>
                    <th style="padding:10px 12px;text-align:right;font-size:0.75rem;text-transform:uppercase;letter-spacing:1px;color:#666;">Unit Price</th>
                    <th style="padding:10px 12px;text-align:right;font-size:0.75rem;text-transform:uppercase;letter-spacing:1px;color:#666;">Subtotal</th>
                </tr>
            </thead>
            <tbody>
                <?php foreach($items as $item): ?>
                <tr style="border-bottom:1px solid #eee;">
                    <td style="padding:10px 12px;font-size:0.85rem;color:#333;"><?php echo htmlspecialchars($item['product_name']); ?></td>
                    <td style="padding:10px 12px;text-align:center;font-size:0.85rem;color:#555;"><?php echo $item['quantity']; ?></td>
                    <td style="padding:10px 12px;text-align:right;font-size:0.85rem;color:#555;">RM <?php echo number_format($item['product_price'],2); ?></td>
                    <td style="padding:10px 12px;text-align:right;font-size:0.85rem;font-weight:600;color:#333;">RM <?php echo number_format($item['subtotal'],2); ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
        ...
        <div style="width:260px;">
            <div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #eee;font-size:0.82rem;color:#666;">
                <span>Subtotal</span><span>RM <?php echo number_format($order['total_amount'],2); ?></span>
            </div>
            <div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #eee;font-size:0.82rem;color:#666;">
                <span>Shipping Fee</span><span>RM <?php echo number_format($order['shipping_fee'],2); ?></span>
            </div>
            <div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #eee;font-size:0.82rem;color:#666;">
                <span>Service Fee</span><span>RM <?php echo number_format($order['service_fee'],2); ?></span>
            </div>
            <div style="display:flex;justify-content:space-between;padding:10px 0 0;font-size:1rem;font-weight:800;color:#b8860b;">
                <span>GRAND TOTAL</span><span>RM <?php echo number_format($order['grand_total'],2); ?></span>
            </div>
        </div>
        </div>

        <!-- FOOTER NOTE -->
        <div style="margin-top:40px;padding-top:20px;border-top:1px solid #ddd;text-align:center;font-size:0.72rem;color:#aaa;">
...
<div class="modal-content" style="background:#121212;border:1px solid #2a2a2a;border-radius:12px;">
    ...
    <div class="modal-body" style="padding:24px;">
        ...
        <div style="margin-bottom:16px;">
            <div style="font-size:0.72rem;color:#555;text-transform:uppercase;letter-spacing:1px;margin-bottom:10px;">Rating</div>
        ...
        <!-- REVIEW TEXT -->
        <div style="margin-bottom:16px;">
            <div style="font-size:0.72rem;color:#555;text-transform:uppercase;letter-spacing:1px;margin-bottom:8px;">Your Review</div>
