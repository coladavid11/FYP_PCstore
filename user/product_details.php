...
.product-id { color: #555; font-size: 0.78rem; letter-spacing: 0.5px; }
.price-tag {
    font-size: 2rem;
    font-weight: 700;
    color: #d4af37;
...
.stock-label span:first-child { color: #888; text-transform: uppercase; letter-spacing: 0.8px; }
.stock-count { font-weight: 600; }
.stock-count.ok    { color: #28a745; }
.stock-count.low   { color: #ffc107; }
.stock-count.empty { color: #dc3545; }

.progress { height: 5px; background: #222; border-radius: 3px; }
.bar-ok    { background: linear-gradient(90deg, #28a745, #5cb85c); }
.bar-low   { background: linear-gradient(90deg, #ffc107, #ffda6a); }
.bar-empty { background: #dc3545; }
...
.qty-wrap button:hover:not(:disabled) { background: #2a2a2a; }
.qty-wrap button:disabled { color: #444; cursor: not-allowed; }
#qty {
...
.subtotal-line span:first-child { color: #888; font-size: 0.85rem; text-transform: uppercase; letter-spacing: 1px; }
.subtotal-line .sub-price { color: #d4af37; font-weight: 700; font-size: 1.2rem; }
...
.btn-wish.not-wishlisted { border-color: #555; color: #aaa; }
.btn-wish.not-wishlisted:hover { border-color: #dc3545; color: #dc3545; background: rgba(220,53,69,0.05); }
.btn-wish.wishlisted { border-color: #dc3545; color: #dc3545; background: rgba(220,53,69,0.1); }
.btn-wish.wishlisted:hover { background: rgba(220,53,69,0.2); }
...
.nav-tabs { border-bottom: 1px solid #2a2a2a; padding: 0 20px; background: #161616; }
.nav-tabs .nav-link {
    color: #666;
    border: none;
...
.nav-tabs .nav-link.active { color: #d4af37; border-bottom-color: #d4af37; background: transparent; }

.tab-content { padding: 24px; }
.tab-pane { color: #ccc; font-size: 0.92rem; line-height: 1.7; }

/* ── SPEC TABLE ── */
.spec-table { width: 100%; border-collapse: collapse; }
.spec-table tr { border-bottom: 1px solid #1e1e1e; }
...
.spec-table td { padding: 12px 16px; color: #ddd; font-size: 0.9rem; }
...
.related-name { font-size: 0.85rem; font-weight: 600; margin-bottom: 4px; line-height: 1.3; }
.related-brand { font-size: 0.73rem; color: #666; margin-bottom: 8px; }
.related-price { color: #d4af37; font-weight: 700; font-size: 0.95rem; }
</style>
</head>
<body>
...
<div class="container py-5">
    ...
    <div class="row g-4 align-items-start">
        ...
        <div class="info-panel">
            ...
            <div class="product-id mb-3"># <?php echo $product['product_id']; ?> &nbsp;·&nbsp; <?php echo htmlspecialchars($product['category_name'] ?? ''); ?></div>

            <div class="price-tag">RM <span id="basePrice"><?php echo number_format($product['price'], 2); ?></span></div>
            ...
            <div class="stock-wrap">
                ...
                <div class="progress">
                    <div class="progress-bar <?php echo $barCls; ?>"
                         style="width:<?php echo max(3,$pct); ?>%"></div>
                </div>
            </div>

            <!-- DESCRIPTION (short preview) -->
            <p class="text-soft" id="descText"
               style="font-size:0.9rem; line-height:1.65; margin-bottom:4px;">
            ...
            <!-- QTY CONTROL -->
            <?php if ($stock > 0): ?>
            <label style="font-size:0.78rem;color:#888;text-transform:uppercase;letter-spacing:1px;display:block;margin-bottom:10px;">
                Quantity
            </label>
            <div class="d-flex align-items-center gap-3 flex-wrap">
                <div class="qty-wrap">
                    ...
                    <input type="number" id="qty" value="1" min="1"
                           max="<?php echo $stock; ?>"
                           readonly>
                    <button type="button" id="btnPlus" onclick="qtyPlus()"
                            <?php echo $stock <= 1 ? 'disabled' : ''; ?>>
            ...
            <?php else: ?>
            <div class="alert" style="background:rgba(220,53,69,0.1);border:1px solid rgba(220,53,69,0.3);color:#dc3545;border-radius:8px;padding:12px 16px;font-size:0.88rem;">
            ...
            <div class="subtotal-line">
                <span>Subtotal</span>
                <span class="sub-price">RM <span id="subtotal"><?php echo number_format($product['price'], 2); ?></span></span>
            </div>
            ...
            <div class="d-flex flex-column gap-2 mt-3">
                ...
                <button id="wishBtn"
                    ...
                    <i id="wishIcon" class="<?php echo $isWishlisted ? 'fa-solid' : 'fa-regular'; ?> fa-heart me-2"></i>
                    <span id="wishText"><?php echo $isWishlisted ? 'Remove from Wishlist' : 'Add to Wishlist'; ?></span>
                </button>
            </div>
