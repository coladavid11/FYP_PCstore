...
.page-hero p { color:#555; font-size:0.82rem; letter-spacing:1.5px; text-transform:uppercase; margin:0; }
...
.stat-pill .sp-icon { color:#d4af37; font-size:1.1rem; width:20px; text-align:center; }
.stat-pill .sp-val  { font-size:1.2rem; font-weight:700; line-height:1; }
.stat-pill .sp-lbl  { font-size:0.72rem; color:#555; text-transform:uppercase; letter-spacing:1px; }
...
.order-meta-item .meta-label { font-size:0.7rem; color:#555; text-transform:uppercase; letter-spacing:1px; }
.order-meta-item .meta-value { font-size:0.88rem; color:#ccc; font-weight:500; }
.meta-total { color:#d4af37 !important; font-size:1rem !important; font-weight:700 !important; }

/* product thumbs strip */
.thumb-strip { display:flex; gap:8px; flex-wrap:wrap; margin-bottom:16px; }
.thumb-strip img {
...
.btn-oa-gold    { background:linear-gradient(45deg,#d4af37,#c5a028); color:#000; }
.btn-oa-gold:hover { background:#fff; color:#000; }
.btn-oa-outline { background:transparent; border:1px solid #2a2a2a; color:#aaa; }
.btn-oa-outline:hover { border-color:#555; color:#fff; }
.btn-oa-danger  { background:rgba(220,53,69,0.1); border:1px solid rgba(220,53,69,0.3); color:#dc3545; }
.btn-oa-danger:hover { background:rgba(220,53,69,0.2); }
...
.filter-tab:hover { border-color:#444; color:#aaa; }
.filter-tab.active { background:#d4af37; border-color:#d4af37; color:#000; font-weight:700; }

/* empty state */
.empty-state { text-align:center; padding:80px 20px; }
.empty-state .es-icon { font-size:3.5rem; color:#1e1e1e; margin-bottom:20px; }
.empty-state h4 { color:#555; margin-bottom:8px; }
.empty-state p  { color:#333; font-size:0.85rem; }
</style>
</head>
<body>
...
<?php else: ?>
    ...
    <div class="row g-3 mb-4">
        <div class="col-6 col-md-3">
            <div class="stat-pill">
                ...
                <div><div class="sp-val"><?php echo $total; ?></div><div class="sp-lbl">Total Orders</div></div>
            </div>
        </div>
        <div class="col-6 col-md-3">
            <div class="stat-pill">
                <div class="sp-icon"><i class="fa fa-truck"></i></div>
                <div><div class="sp-val"><?php echo $ongoing; ?></div><div class="sp-lbl">Ongoing</div></div>
            </div>
        </div>
        <div class="col-6 col-md-3">
            <div class="stat-pill">
                <div class="sp-icon"><i class="fa fa-check-circle"></i></div>
                <div><div class="sp-val"><?php echo $completed; ?></div><div class="sp-lbl">Completed</div></div>
            </div>
        </div>
        <div class="col-6 col-md-3">
            <div class="stat-pill">
                <div class="sp-icon"><i class="fa fa-coins"></i></div>
                <div><div class="sp-val" style="font-size:1rem;">RM <?php echo number_format($spent,0); ?></div><div class="sp-lbl">Total Spent</div></div>
            </div>
        </div>
    </div>
    ...
    <?php else: ?>
        ...
        <div class="filter-tabs" id="filterTabs">
            ...
            <button class="filter-tab" data-filter="processing">Processing (<?php echo count(array_filter($orders,fn($o)=>strtolower($o['order_status'])==='processing')); ?>)</button>
            ...
            <div class="order-body">
                ...
                <div class="order-meta">
                    <div class="order-meta-item">
                        ...
                        <span class="meta-value"><?php echo date('d M Y', strtotime($order['created_at'])); ?></span>
                    </div>
                    <div class="order-meta-item">
                        <span class="meta-label">Items</span>
                        <span class="meta-value"><?php echo $totalItems; ?> item<?php echo $totalItems != 1 ? 's' : ''; ?></span>
                    </div>
                    <div class="order-meta-item">
                    ...
                    <div class="order-meta-item">
                        ...
                        <span class="meta-value meta-total">RM <?php echo number_format($order['grand_total'],2); ?></span>
                    </div>
                </div>
                ...
                <div style="margin-bottom:14px;">
                    <div style="font-size:0.72rem;color:#555;text-transform:uppercase;letter-spacing:1px;margin-bottom:6px;">Items</div>
                    <?php foreach($thumbItems as $ti): ?>
                    <div style="font-size:0.82rem;color:#888;margin-bottom:2px;">
                        <i class="fa fa-circle" style="font-size:0.4rem;color:#333;vertical-align:middle;margin-right:6px;"></i>
                        <?php echo htmlspecialchars($ti['product_name']); ?>
                        <span style="color:#555;"> × <?php echo $ti['quantity']; ?></span>
                    </div>
                    <?php endforeach; ?>
                    <?php if ($totalItems > 3): ?>
                        <div style="font-size:0.75rem;color:#444;margin-top:4px;">+<?php echo $totalItems - 3; ?> more item(s)</div>
                    <?php endif; ?>
                </div>
                <?php endif; ?>
                ...
                <div class="order-actions">
                    ...
                    <?php if (strtolower($order['order_status']) === 'completed'): ?>
                        <button class="btn-oa btn-oa-outline"
