...
if ($game_id <= 0) { header('Location: game_check.php'); exit(); }

/* ── FETCH GAME ── */
$stmt = $dbh->prepare("SELECT * FROM tblgamecheck WHERE game_id = ? AND status = 1 LIMIT 1");
$stmt->execute([$game_id]);
$game = $stmt->fetch(PDO::FETCH_ASSOC);
if (!$game) { header('Location: game_check.php'); exit(); }
...
if (!empty($game['rec_category_ids'])) {
    ...
    if (!empty($catIds)) {
        ...
        foreach ($allProds as $p) {
            ...
            if (count($grouped[$cname]) < 3) $grouped[$cname][] = $p;
        }
        $recProducts = $grouped;
    }
}
...
/* ── BREADCRUMB ── */
.breadcrumb-dark { display:flex; gap:8px; align-items:center; margin-bottom:28px; flex-wrap:wrap; }
.breadcrumb-dark a { color:#555; text-decoration:none; font-size:0.78rem; transition:color 0.2s; }
.breadcrumb-dark a:hover { color:#d4af37; }
.breadcrumb-dark .sep { color:#222; font-size:0.6rem; }
.breadcrumb-dark .cur { color:#888; font-size:0.78rem; }
...
.game-hero-meta { display:flex; gap:20px; flex-wrap:wrap; align-items:center; }
.hero-meta-item { font-size:0.78rem; color:#888; display:flex; align-items:center; gap:5px; }
.hero-meta-item i { color:#d4af37; font-size:0.7rem; }
...
.sc-header {
    padding:10px 14px; font-size:0.7rem;
    text-transform:uppercase; letter-spacing:1px;
    font-weight:700; text-align:center;
}
.sc-header.min-col { background:#1a1a1a; color:#888; }
.sc-header.rec-col { background:rgba(212,175,55,0.12); color:#d4af37; border-left:1px solid rgba(212,175,55,0.2); }
.sc-header.label-col { background:#161616; color:#555; text-align:left; }
...
.sc-cell.label-col i { color:#d4af37; width:14px; text-align:center; }
.sc-cell.min-col { background:#1a1a1a; color:#ccc; }
.sc-cell.rec-col {
    background:rgba(212,175,55,0.06); color:#e8c84a;
    font-weight:600; border-left:1px solid rgba(212,175,55,0.15);
}
...
/* ── DESCRIPTION ── */
.game-desc { font-size:0.9rem; color:#999; line-height:1.75; }

/* ── PRODUCT CARD ── */
.prod-card {
    background:#161616; border:1px solid #1e1e1e;
    border-radius:10px; overflow:hidden;
    transition:all 0.25s; text-decoration:none; color:#fff;
    display:block; height:100%;
}
.prod-card:hover {
    border-color:#d4af37; transform:translateY(-5px);
    box-shadow:0 12px 30px rgba(212,175,55,0.15); color:#fff;
...
.prod-cat { font-size:0.68rem; color:#555; text-transform:uppercase; letter-spacing:0.8px; margin-bottom:3px; }
.prod-name {
    font-size:0.85rem; font-weight:600; color:#ddd;
    margin-bottom:8px; line-height:1.3;
    display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden;
}
.prod-price { color:#d4af37; font-weight:700; font-size:0.95rem; }
.btn-prod {
...
.btn-prod:hover { background:#fff; color:#000; }

/* ── SECTION TITLE ── */
.section-eyebrow {
    font-size:0.7rem; color:#d4af37; letter-spacing:2px;
    text-transform:uppercase; margin-bottom:6px;
}
.section-h { font-family:'Playfair Display',serif; font-size:1.6rem; color:#fff; margin-bottom:4px; }
.section-sub { font-size:0.8rem; color:#555; margin-bottom:24px; }
...
.nav-arrow:hover { border-color:#d4af37; color:#d4af37; }
.nav-arrow .arrow-label { font-size:0.65rem; color:#555; text-transform:uppercase; letter-spacing:1px; }
.nav-arrow .arrow-name { font-weight:600; color:#ddd; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
...
.info-chip i { color:#d4af37; font-size:0.9rem; width:16px; text-align:center; }
.info-chip .ic-lbl { font-size:0.65rem; color:#555; text-transform:uppercase; letter-spacing:1px; margin-bottom:2px; }
.info-chip .ic-val { font-size:0.85rem; color:#ccc; font-weight:500; }
</style>
</head>
<body>
...
<div class="container py-5">
    ...
    <div class="game-hero">
        ...
        <div class="game-hero-content">
            ...
            <div class="game-hero-meta">
                ...
                <span class="hero-meta-item"><i class="fa fa-building"></i> <?php echo htmlspecialchars($game['developer']); ?></span>
                <?php endif; ?>
                <?php if ($game['release_year']): ?>
                <span class="hero-meta-item"><i class="fa fa-calendar"></i> <?php echo $game['release_year']; ?></span>
                <?php endif; ?>
                <?php if ($game['genre']): ?>
                <span class="hero-meta-item"><i class="fa fa-tag"></i> <?php echo htmlspecialchars($game['genre']); ?></span>
                <?php endif; ?>
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-8">
            ...
            <div class="panel">
                ...
                <div class="spec-compare">
                    ...
                    <div class="sc-cell rec-col"><?php echo htmlspecialchars($game['rec_cpu'] ?? $game['min_cpu']); ?></div>
                    ...
                    <div class="sc-cell rec-col"><?php echo htmlspecialchars($game['rec_ram'] ?? $game['min_ram']); ?></div>
                    ...
                    <div class="sc-cell rec-col"><?php echo htmlspecialchars($game['rec_gpu'] ?? $game['min_gpu']); ?></div>
                    ...
                    <div class="sc-cell rec-col"><?php echo htmlspecialchars($game['rec_storage'] ?? $game['min_storage']); ?></div>
                </div>

                <!-- Note -->
                <p style="font-size:0.75rem;color:#444;margin-top:14px;margin-bottom:0;">
                    <i class="fa fa-circle-info me-1" style="color:#555;"></i>
                    Minimum = playable at 1080p medium settings. Recommended = smooth 1080p high/ultra or 1440p medium.
                </p>
            </div>
        ...
        <div class="col-lg-4">
            ...
            <div class="panel">
                ...
                <div class="d-flex flex-column gap-2">
                    ...
                    <div class="info-chip">
                        ...
                        <div><div class="ic-lbl">Developer</div><div class="ic-val"><?php echo htmlspecialchars($game['developer']); ?></div></div>
                    </div>
                    <?php endif; ?>
                    <?php if ($game['release_year']): ?>
                    <div class="info-chip">
                        <i class="fa fa-calendar-days"></i>
                        <div><div class="ic-lbl">Release Year</div><div class="ic-val"><?php echo $game['release_year']; ?></div></div>
                    </div>
                    <?php endif; ?>
                    <?php if ($game['genre']): ?>
                    <div class="info-chip">
                        <i class="fa fa-gamepad"></i>
                        <div><div class="ic-lbl">Genre</div><div class="ic-val"><?php echo htmlspecialchars($game['genre']); ?></div></div>
                    </div>
                    <?php endif; ?>
                    <div class="info-chip">
                        <i class="fa <?php echo $demand['icon']; ?>" style="color:<?php echo $demand['color']; ?>;"></i>
                        <div>
                            <div class="ic-lbl">Hardware Demand</div>
                            <div class="ic-val" style="color:<?php echo $demand['color']; ?>;"><?php echo $demand['label']; ?></div>
                        </div>
                    </div>
                    <div class="info-chip">
                        <i class="fa fa-hard-drive"></i>
                        <div><div class="ic-lbl">Min Storage Needed</div><div class="ic-val"><?php echo htmlspecialchars($game['min_storage']); ?></div></div>
                    </div>
                </div>
            </div>
            ...
            <div class="panel" style="background:#0d0d0d;">
                ...
                <div style="display:flex;gap:10px;align-items:flex-start;padding:9px 0;border-bottom:1px solid #151515;">
                    <div style="width:28px;height:28px;border-radius:6px;background:#141414;border:1px solid #1e1e1e;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                        <i class="<?php echo $icon; ?>" style="color:#d4af37;font-size:0.7rem;"></i>
                    </div>
                    <div>
                        <div style="font-size:0.65rem;color:#444;text-transform:uppercase;letter-spacing:0.8px;margin-bottom:1px;"><?php echo $lbl; ?></div>
                        <div style="font-size:0.8rem;color:#bbb;font-weight:500;"><?php echo htmlspecialchars($val); ?></div>
                    </div>
                </div>
                <?php endforeach; ?>
    ...
    <div class="mt-5">
        ...
        <p class="section-sub">Products from our store that meet or exceed the requirements for <strong style="color:#d4af37;"><?php echo htmlspecialchars($game['name']); ?></strong></p>
        ...
        <div class="mb-4">
            ...
            <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;">
                ...
                <h5 style="margin:0;font-size:1rem;color:#ddd;font-weight:600;"><?php echo htmlspecialchars($categoryName); ?></h5>
            ...
            <div class="col">
                <a href="product_details.php?id=<?php echo $p['product_id']; ?>" class="prod-card">
                    ...
                    <div class="prod-body">
                        ...
                        $specLines = [];
                        if (!empty($p['cpu']))  $specLines[] = ['fa fa-microchip','CPU', $p['cpu']];
                        if (!empty($p['gpu']))  $specLines[] = ['fa fa-display','GPU', $p['gpu']];
                        if (!empty($p['ram']))  $specLines[] = ['fa fa-memory','RAM', $p['ram']];
                        if (!empty($p['storage'])) $specLines[] = ['fa fa-hard-drive','SSD', $p['storage']];
                        ...
                        <div style="display:flex;gap:6px;align-items:baseline;margin-bottom:3px;">
                            <i class="<?php echo $icon; ?>" style="color:#555;font-size:0.65rem;width:12px;text-align:center;flex-shrink:0;"></i>
                            <span style="font-size:0.72rem;color:#555;"><?php echo $lbl; ?>:</span>
                            <span style="font-size:0.72rem;color:#888;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"><?php echo htmlspecialchars($val); ?></span>
                        </div>
                        <?php endforeach; ?>
    ...
    <div class="game-nav">
        ...
        <?php else: ?>
        <div class="nav-arrow disabled"><i class="fa fa-chevron-left"></i><div><div class="arrow-label">Previous</div><div class="arrow-name">—</div></div></div>
        <?php endif; ?>

        <a href="game_check.php" style="font-size:0.78rem;color:#555;text-decoration:none;white-space:nowrap;padding:8px 16px;border-radius:8px;border:1px solid #1e1e1e;" onmouseover="this.style.color='#d4af37'" onmouseout="this.style.color='#555'">
            <i class="fa fa-grid-2 me-1"></i> All Games
        </a>

        <?php if ($nextGame): ?>
        <a href="game_check_detail.php?id=<?php echo $nextGame['game_id']; ?>" class="nav-arrow" style="flex-direction:row-reverse;text-align:right;">
        ...
        <?php else: ?>
        <div class="nav-arrow disabled" style="flex-direction:row-reverse;"><i class="fa fa-chevron-right"></i><div><div class="arrow-label">Next</div><div class="arrow-name">—</div></div></div>
        <?php endif; ?>
    </div>
