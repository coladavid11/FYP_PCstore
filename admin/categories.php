...
if (isset($_GET['delete'])) {
    ...
    if ($usedCount > 0) {
        header("Location: categories.php?error=in_use&count=$usedCount&" . http_build_query([
            'search' => $_GET['search'] ?? '',
    ...
    $del = $dbh->prepare("DELETE FROM categories WHERE category_id = :id");
...
.sort-select {
    ...
    font-family:'Poppins',sans-serif; cursor:pointer; transition:border-color 0.2s; min-width:160px;
}
...
.cat-id    { color:#bbb; font-size:0.78rem; }
...
<div class="main">
    ...
    <div class="topbar">
        <div>
            <h1><i class="fa fa-folder-open" style="color:#d4af37;margin-right:8px;font-size:1.3rem;"></i>Categories</h1>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th style="width:32%;">Category Name</th>
                    <th style="width:13%;">Status</th>
                    <th style="width:16%;">Products</th>
            ...
            <?php if (!empty($categories)): ?>
                ...
                <tr>
                    ...
                    <td><span class="date-text"><?php echo date('d M Y', strtotime($cat->created_at)); ?></span></td>
                    ...
                    <td>
                        ...
                        <a href="javascript:void(0);"
                           ...
                           onclick="warnInUse('<?php echo htmlspecialchars(addslashes($cat->category_name)); ?>', <?php echo $cat->product_count; ?>, 'category')">
                            <i class="fa fa-trash"></i> Delete
                        </a>
                        <?php else: ?>
                        <a href="javascript:void(0);"
                           class="action-btn delete"
                           onclick="confirmDelete('categories.php?delete=<?php echo $cat->category_id; ?>&sort=<?php echo $sort; ?>&search=<?php echo urlencode($search); ?>', '<?php echo htmlspecialchars(addslashes($cat->category_name)); ?>')">
                            <i class="fa fa-trash"></i> Delete
                        </a>
                        <?php endif; ?>
                    </td>
                ...
                <tr>
                    <td colspan="6">
                        <div class="empty-state">
                            ...
                            <p>No categories found<?php echo $search !== '' ? ' for "'.htmlspecialchars($search).'"' : ''; ?>.</p>
                            <?php if($search !== ''): ?>
