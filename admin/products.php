...
foreach ($categories as $c) { if ($c->category_id == $filterCat)   $selectedCatName   = $c->category_name; }
foreach ($brands     as $b) { if ($b->brand_id    == $filterBrand) $selectedBrandName = $b->brand_name; }
...
.topbar {
    ...
    margin-bottom:20px; background:#fff; padding:15px 25px;
    border-radius:4px; box-shadow:0 2px 8px rgba(0,0,0,0.05);
}
.topbar h1 { font-size:1.8rem; color:#111; font-weight:600; }
.topbar-links { display:flex; gap:15px; align-items:center; }
.Back { text-decoration:none; font-weight:500; color:#d4af37; font-size:0.95rem; transition:0.3s; }
...
.btn-add {
    ...
    font-size:0.9rem; border:1px solid #d4af37; transition:0.3s;
}
...
.search-wrap i {
    ...
    transform:translateY(-50%); color:#bbb; font-size:0.85rem;
    pointer-events:none;
}
.search-input {
...
.filter-dropdown .dd-toggle {
    ...
    background:#fff; font-size:0.85rem; color:#555; cursor:pointer;
    transition:border-color 0.2s; white-space:nowrap; user-select:none;
    font-family:'Poppins',sans-serif;
}
.filter-dropdown .dd-toggle:hover,
.filter-dropdown .dd-toggle.open { border-color:#d4af37; }
.filter-dropdown .dd-toggle.active-filter { color:#d4af37; font-weight:600; border-color:#d4af37; }
.filter-dropdown .dd-toggle i.arrow { font-size:0.65rem; color:#bbb; transition:transform 0.2s; }
.filter-dropdown .dd-toggle.open i.arrow { transform:rotate(180deg); }

.filter-dropdown .dd-menu {
    display:none; position:absolute; top:calc(100% + 4px); left:0; right:0;
    background:#fff; border:1px solid #e0e0e0; border-radius:4px;
    box-shadow:0 6px 20px rgba(0,0,0,0.1); z-index:100;
    max-height:260px; overflow-y:auto;
}
.filter-dropdown .dd-menu.show { display:block; }
.filter-dropdown .dd-menu::-webkit-scrollbar { width:4px; }
.filter-dropdown .dd-menu::-webkit-scrollbar-thumb { background:#e0e0e0; border-radius:2px; }

.filter-dropdown .dd-item {
    padding:9px 14px; font-size:0.83rem; color:#555; cursor:pointer;
    transition:background 0.15s; display:flex; align-items:center; gap:8px;
}
.filter-dropdown .dd-item:hover { background:#fafafa; color:#111; }
.filter-dropdown .dd-item.selected { color:#d4af37; font-weight:600; background:#fffdf0; }
.filter-dropdown .dd-divider { border:none; border-top:1px solid #f0f0f0; margin:3px 0; }

/* Filter action buttons */
.btn-filter {
    padding:9px 18px; border-radius:4px; font-size:0.84rem;
    font-weight:500; cursor:pointer; transition:all 0.2s;
    border:none; font-family:'Poppins',sans-serif; white-space:nowrap;
}
.btn-filter-go { background:#000; color:#d4af37; border:1px solid #000; }
.btn-filter-go:hover { background:#d4af37; color:#000; border-color:#d4af37; }
.btn-filter-reset { background:#fff; color:#888; border:1px solid #e0e0e0; }
.btn-filter-reset:hover { border-color:#aaa; color:#555; }

/* Active filter chips */
.filter-chips { display:flex; gap:8px; flex-wrap:wrap; margin-top:12px; }
.chip {
    display:inline-flex; align-items:center; gap:5px;
    background:#fffdf0; border:1px solid rgba(212,175,55,0.4);
    color:#b89a2e; border-radius:20px; padding:3px 10px;
    font-size:0.75rem; font-weight:500; text-decoration:none;
}
.chip i { font-size:0.6rem; }
.chip:hover { background:#fff8d6; color:#9a800a; }

/* ── TABLE BOX ── */
.table-box {
    background:#fff; padding:24px 30px;
    box-shadow:0 2px 8px rgba(0,0,0,0.05); border-radius:4px;
}
...
.table-meta .result-count { font-size:0.82rem; color:#aaa; }
.table-meta .result-count strong { color:#333; }

table { width:100%; border-collapse:collapse; }
table th, table td { padding:13px 12px; border-bottom:1px solid #f0f0f0; text-align:left; vertical-align:middle; }
table th {
    color:#d4af37; font-weight:600; background:#fafafa;
...
.no-img {
    width:52px; height:52px; background:#f5f5f5; border:1px solid #eee;
    border-radius:4px; display:flex; align-items:center; justify-content:center;
    color:#ccc; font-size:0.7rem;
}

.badge-cat {
    display:inline-block; background:#f5f5f5; color:#777;
    font-size:0.72rem; padding:3px 8px; border-radius:3px;
}
.badge-brand {
    display:inline-block; background:#fffbeb; color:#b89a2e;
    font-size:0.72rem; padding:3px 8px; border-radius:3px;
...
.action-btn { text-decoration:none; font-weight:500; font-size:0.85rem; transition:0.2s; }
...
.empty-state i { font-size:2.5rem; color:#ddd; margin-bottom:16px; display:block; }
.empty-state p { color:#aaa; font-size:0.9rem; }
.empty-state a { color:#d4af37; font-size:0.85rem; }
</style>
</head>
<body>
    ...
    <div class="filter-bar">
        ...
        <div class="filter-row">
            ...
            <div class="filter-dropdown" id="ddCat">
                <div class="dd-toggle <?php echo $filterCat > 0 ? 'active-filter' : ''; ?>" onclick="toggleDd('ddCat')">
                    <span id="ddCatLabel">
            ...
            <div class="filter-dropdown" id="ddBrand">
                <div class="dd-toggle <?php echo $filterBrand > 0 ? 'active-filter' : ''; ?>" onclick="toggleDd('ddBrand')">
    ...
    <div class="table-box">
        ...
        <div class="table-meta">
            <div class="result-count">
                Showing <strong><?php echo $totalCount; ?></strong> product<?php echo $totalCount !== 1 ? 's' : ''; ?>
                <?php if ($search !== '' || $filterCat > 0 || $filterBrand > 0): ?>
        ...
        <table>
            ...
            <tr>
                ...
                <td style="max-width:200px;">
                    <div style="font-weight:500;color:#222;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">
                        <?php
                ...
                <td>
                    ...
                    if ($stock <= 0)   { $cls = 'stock-out'; $label = 'Out of Stock'; }
                    elseif ($stock < 5){ $cls = 'stock-low'; $label = "Low ($stock)"; }
                    else               { $cls = 'stock-ok';  $label = $stock; }
                    ?>
            ...
            <tr>
                <td colspan="9">
                    <div class="empty-state">
                        ...
                        <p>No products found<?php echo ($search !== '' || $filterCat > 0 || $filterBrand > 0) ? ' matching your filters' : ''; ?>.</p>
                        <?php if ($search !== '' || $filterCat > 0 || $filterBrand > 0): ?>
