...
.stat-card.active-filter { border-bottom-color:#d4af37; }
.stat-card-label { font-size:0.7rem; font-weight:600; color:#aaa; text-transform:uppercase; letter-spacing:0.5px; margin-bottom:6px; display:flex; align-items:center; gap:6px; }
.stat-card-label i { font-size:0.8rem; }
.stat-card-num   { font-size:1.7rem; font-weight:700; color:#222; line-height:1; }
.stat-card-sub   { font-size:0.7rem; color:#bbb; margin-top:4px; }

/* colour accents per status */
.stat-card.s-all          { border-bottom-color:#d4af37; }
.stat-card.s-processing   .stat-card-label i { color:#3498db; }
.stat-card.s-packed       .stat-card-label i { color:#9b59b6; }
.stat-card.s-shipped      .stat-card-label i { color:#f39c12; }
.stat-card.s-delivered    .stat-card-label i { color:#27ae60; }
.stat-card.s-cancelled    .stat-card-label i { color:#e74c3c; }
...
.sort-select {
    ...
    font-family:'Poppins',sans-serif; cursor:pointer; transition:border-color 0.2s; min-width:170px;
}
...
.filter-pills span { font-size:0.78rem; color:#aaa; }
.fpill {
    display:inline-flex; align-items:center; gap:5px;
    padding:5px 12px; border-radius:20px; font-size:0.75rem;
    font-weight:500; border:1px solid #e0e0e0; color:#777;
    text-decoration:none; transition:0.15s; cursor:pointer;
}
.fpill:hover { border-color:#bbb; }
.fpill.fp-active          { background:#d4af37; border-color:#d4af37; color:#000; font-weight:700; }
.fpill.fp-processing      { }
.fpill.fp-processing.on   { background:#dbeafe; border-color:#3498db; color:#1a5276; }
.fpill.fp-packed.on       { background:#ede9fe; border-color:#9b59b6; color:#5b2c8d; }
.fpill.fp-shipped.on      { background:#fef9c3; border-color:#f39c12; color:#7d4e00; }
.fpill.fp-delivered.on    { background:#d4edda; border-color:#27ae60; color:#155724; }
.fpill.fp-cancelled.on    { background:#fde8e8; border-color:#e74c3c; color:#7b0000; }
...
.order-num  { font-weight:600; color:#222; font-size:0.83rem; }
.order-id   { color:#bbb; font-size:0.75rem; }
.date-text  { font-size:0.78rem; color:#aaa; }
.cust-name  { font-weight:500; color:#333; }
.cust-email { font-size:0.75rem; color:#aaa; }
.amount-val { font-weight:600; color:#222; }
...
<div class="main">
    ...
    <div class="topbar">
        <div>
            <h1><i class="fa fa-cart-shopping" style="color:#d4af37;margin-right:8px;font-size:1.3rem;"></i>Orders</h1>
    ...
    <div class="stat-grid">
        ...
        <a href="orders.php" class="stat-card s-all <?php echo $filterSt==='' && $search==='' ? 'active-filter' : ''; ?>">
        ...
        <a href="orders.php?status=processing" class="stat-card s-processing <?php echo $filterSt==='processing' ? 'active-filter' : ''; ?>">
        ...
        <a href="orders.php?status=packed" class="stat-card s-packed <?php echo $filterSt==='packed' ? 'active-filter' : ''; ?>">
        ...
        <a href="orders.php?status=shipped" class="stat-card s-shipped <?php echo $filterSt==='shipped' ? 'active-filter' : ''; ?>">
        ...
        <a href="orders.php?status=delivered" class="stat-card s-delivered <?php echo $filterSt==='delivered' ? 'active-filter' : ''; ?>">
        ...
        <a href="orders.php?status=cancelled" class="stat-card s-cancelled <?php echo $filterSt==='cancelled' ? 'active-filter' : ''; ?>">
    ...
    <div class="filter-bar">
        ...
        <div class="filter-pills">
            <span>Status:</span>
            <a class="fpill <?php echo $filterSt==='' ? 'fp-active' : ''; ?>"
               onclick="setStatus('')">All</a>
            <?php
            $pillDefs = [
            ...
            foreach ($pillDefs as $val => [$cls, $ico, $lbl]):
                $on = $filterSt === $val ? 'on' : '';
            ?>
            <a class="fpill <?php echo $cls; ?> <?php echo $on; ?>"
               onclick="setStatus('<?php echo $val; ?>')">
        ...
        <select name="sort" class="sort-select" onchange="this.form.submit()">
            ...
            <option value="amount_hi" <?php echo $sort==='amount_hi' ?'selected':''; ?>>Amount: High → Low</option>
            <option value="amount_lo" <?php echo $sort==='amount_lo' ?'selected':''; ?>>Amount: Low → High</option>
        </select>
            ...
            <?php if (!empty($orders)): ?>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <div style="font-size:0.72rem;color:#ccc;"><?php echo date('h:i A', strtotime($o->created_at)); ?></div>
                    </td>
                ...
                <tr>
                    <td colspan="8">
                        <div class="empty-state">
                            ...
                            <p>No orders found<?php echo ($search !== '' || $filterSt !== '') ? ' for the current filters' : ''; ?>.</p>
