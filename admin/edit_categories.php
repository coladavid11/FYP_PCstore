...
/* ── FETCH CATEGORY ── */
$fetch = $dbh->prepare("SELECT * FROM categories WHERE category_id = :id");
$fetch->bindParam(':id', $id, PDO::PARAM_INT);
$fetch->execute();
$cat = $fetch->fetch(PDO::FETCH_OBJ);
...
/* ── FETCH PRODUCT COUNT ── */
$countQ = $dbh->prepare("SELECT COUNT(*) FROM products WHERE category_id = :id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $newDesc   = trim($_POST['category_description'] ?? '');
    ...
    if (empty($errors)) {
        ...
        if ($dup->fetch()) {
            $dupName = true;
            $errors[] = "A category named \"$newName\" already exists.";
        }
    }
    ...
    if (empty($errors)) {
        $upd = $dbh->prepare(
            "UPDATE categories
             SET category_name = :name, category_description = :desc, status = :status
             WHERE category_id = :id"
        );
        $upd->bindParam(':name',   $newName);
        $upd->bindParam(':desc',   $newDesc);
        $upd->bindParam(':status', $newStatus);
        $upd->bindParam(':id',     $id, PDO::PARAM_INT);
        $upd->execute();
        $success = true;
        $cat->category_name        = $newName;
        $cat->category_description = $newDesc;
...
.char-count {
    ...
    font-size:0.7rem; color:#bbb; pointer-events:none;
}
...
<div class="main">
    ...
    <div class="topbar">
        <div>
            ...
            <div class="topbar-sub">Update category name, description and status</div>
        </div>
        <div class="topbar-right">
            <a href="categories.php" class="btn-back"><i class="fa fa-arrow-left"></i> Back to Categories</a>
        </div>
    </div>
    ...
    <div class="edit-layout">
        ...
        <div class="form-card">
            <div class="form-card-header">
                ...
                <span class="badge-id">ID #<?php echo $cat->category_id; ?></span>
            </div>
            ...
            <div class="form-body">
                ...
                <div class="form-group">
                    ...
                    <input
                        ...
                        id="category_name"
                        name="category_name"
                        class="form-control <?php echo $dupName ? 'is-error' : (!empty($errors) && trim($_POST['category_name'] ?? '') === '' ? 'is-error' : ''); ?>"
                        value="<?php echo htmlspecialchars($cat->category_name); ?>"
                        maxlength="100"
                        autocomplete="off"
                        placeholder="e.g. Graphics Card, Gaming Laptop"
                    >
                    <div class="form-hint">Maximum 100 characters. Name must be unique.</div>
                    <?php if ($dupName): ?>
                    <div class="field-error"><i class="fa fa-circle-exclamation"></i> A category with this name already exists.</div>
                    <?php elseif (!empty($errors) && trim($_POST['category_name'] ?? '') === ''): ?>
                    <div class="field-error"><i class="fa fa-circle-exclamation"></i> Category name cannot be empty.</div>
                    <?php endif; ?>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="input-wrap">
                        ...
                        ><?php echo htmlspecialchars($cat->category_description ?? ''); ?></textarea>
                    ...
                    <div class="status-options">
                        <div class="status-option">
                            <input type="radio" id="status_active" name="status" value="active"
                                <?php echo $cat->status === 'active' ? 'checked' : ''; ?>>
                        ...
                        <div class="status-option">
                            <input type="radio" id="status_inactive" name="status" value="inactive"
                                <?php echo $cat->status === 'inactive' ? 'checked' : ''; ?>>
                            <label for="status_inactive" class="label-inactive">
                                <i class="fa fa-circle-xmark"></i> Inactive
                            </label>
                        </div>
                    </div>
                    <div class="form-hint">Inactive categories are hidden from the storefront.</div>
                    <div class="inactive-warning" id="inactiveWarn">
                        <i class="fa fa-triangle-exclamation"></i>
                        <strong>Note:</strong> This category has <strong><?php echo $productCount; ?></strong>
            ...
            <div class="side-card">
                ...
                <div class="side-card-body">
                    <div class="meta-row">
                        ...
                        <span>ID: <strong>#<?php echo $cat->category_id; ?></strong></span>
                    </div>
                    <div class="meta-row">
                        <i class="fa fa-calendar"></i>
                        <div>
                            <div style="margin-bottom:2px;">Created</div>
                            <strong><?php echo date('d M Y', strtotime($cat->created_at)); ?></strong>
                        </div>
                    </div>
                    <div class="meta-row">
            ...
            <div class="side-card">
                ...
                <div class="side-card-body">
                    <div class="meta-row">
                        ...
                        <div>
                            <div style="margin-bottom:6px;font-size:0.8rem;">Total products using this category</div>
                    ...
                    <div style="margin-top:12px;padding-top:12px;border-top:1px solid #f0f0f0;">
                        <a href="products.php?category=<?php echo $id; ?>" style="font-size:0.8rem;color:#d4af37;text-decoration:none;display:inline-flex;align-items:center;gap:5px;">
...
function updateCount() {
    ...
    descCount.style.color = len > 450 ? '#e74c3c' : '#bbb';
}
...
Swal.fire({
    ...
    html: `A category named <b style="color:#d4af37;"><?php echo htmlspecialchars(addslashes($_POST['category_name'])); ?></b> already exists.<br>
